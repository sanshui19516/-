...
// 修改：同时控制三个新条目的开关（UID 175、176、177）
// ============================================================
...
(function() {
    ...
    // 修改点：从单个 UID 改为三个新条目的 UID 数组
    const ENTRY_UIDS = [175, 176, 177];  // 强上判定、核心基调、角色详细规则
    ...
    // ─── 获取第一个条目的状态（用于判断当前是开还是关）─────
    async function getCurrentState() {
        try {
            const wb = await TavernHelper.getWorldbook(WORLD_BOOK_NAME);
            if (!wb) return false;
            const firstEntry = wb.find(e => e.uid === ENTRY_UIDS[0]);
            return firstEntry ? firstEntry.enabled : false;
        } catch (e) {
            console.error('[真实世界] 获取状态失败:', e);
            return false;
        }
    }
    ...
    async function toggleEntry() {
        try {
            ...
            // 读取第一个条目的当前状态
            const firstEntry = wb.find(e => e.uid === ENTRY_UIDS[0]);
            if (!firstEntry) { toastr.error('未找到 UID: ' + ENTRY_UIDS[0]); return; }

            const currentState = firstEntry.enabled;
            const newState = !currentState;

            // 循环遍历所有 UID，统一修改状态
            let foundCount = 0;
            ENTRY_UIDS.forEach(uid => {
                const entry = wb.find(e => e.uid === uid);
                if (entry) {
                    entry.enabled = newState;
                    foundCount++;
                } else {
                    console.warn('[真实世界] 未找到 UID:', uid);
                }
            });

            if (foundCount === 0) {
                toastr.error('未找到任何有效条目');
                return;
            }
            ...
            // 显示通知
            showGlassNotification(newState ? 'on' : 'off');
            ...
            console.log('[真实世界] → 已' + (newState ? '启用' : '禁用') + ' UID: ' + ENTRY_UIDS.join(', '));
        } catch (e) {
    ...
    (async function init() {
        try {
            ...
            const wb = await TavernHelper.getWorldbook(WORLD_BOOK_NAME);
            if (!wb) {
                toastr.error('未找到世界书: ' + WORLD_BOOK_NAME);
                return;
            }

            const firstEntry = wb.find(e => e.uid === ENTRY_UIDS[0]);
            if (!firstEntry) {
                toastr.error('未找到 UID: ' + ENTRY_UIDS[0] + '，请检查条目UID是否正确');
                return;
            }
            ...
            console.log('[真实世界] 已就绪, 控制 UID: ' + ENTRY_UIDS.join(', ') + ' → 当前状态: ' + (currentState ? '启用' : '禁用'));
        } catch (e) {
