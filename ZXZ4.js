        ...
        try {
            if (typeof eventOn !== 'undefined' && typeof Mvu !== 'undefined') {
                eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, (newVars, oldVars) => {
                    ...
                    const newEvents = (typeof _ !== 'undefined' 
                        ? _.get(newVars, 'stat_data.事件') 
                        : newVars?.stat_data?.['事件']);
                    const oldEvents = (typeof _ !== 'undefined' 
                        ? _.get(oldVars, 'stat_data.事件') 
                        : oldVars?.stat_data?.['事件']);

                    if (!newEvents || !oldEvents) return;

                    if (newEvents.雨天共伞 === '已发生' && oldEvents.雨天共伞 !== '已发生') {
                        const name = '林夕';
                        if (chatData[name]) {
                            setTimeout(() => {
                                postActivePost(name);
                    ...
                    const newLinxi = (typeof _ !== 'undefined' 
                        ? _.get(newVars, 'stat_data.林夕') 
                        : newVars?.stat_data?.['林夕']);
                    const oldLinxi = (typeof _ !== 'undefined' 
                        ? _.get(oldVars, 'stat_data.林夕') 
                        : oldVars?.stat_data?.['林夕']);
                ...
                console.log('[事件触发] MVU监听已注册');
            }
        } catch (e) {
            console.warn('[事件触发] 注册MVU监听失败:', e);
        }
    }
        ...
        (role.messages || []).forEach(msg => {
            ...
            msgContainer.append(`
                <div class="message-bubble ${isMe ? 'self' : ''}" style="
                    ...
                    background:${isMe ? '#06c755' : 'var(--bg-card,#2a2a3e)'};
                    color:${isMe ? '#fff' : 'var(--text-primary,#ece8e0)'};
