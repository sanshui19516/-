// ============================================================
// 都市回廊 · 地图系统（高度修复版）
// 功能：L1大地图 → L2地点详情 → L3子场景/人物深度详情
// 绑定：世界.当前场景、各事件解锁变量
// 风格：HD-2D / RPG / 霓虹（可切换）
// 适配：手机 / 平板 / 电脑
// 修复：仅修复地图容器高度问题，保留所有原有数据
// ============================================================
...
(function() {
    ...
    const CONFIG = {
        ...
        VAR_CINEMA_UNLOCKED: '_通用.电影院已解锁',
        VAR_BEACH_UNLOCKED: '_通用.海边已解锁',
        THEME: 'octopath',
        NODE_SIZE: 28,
        COLORS: {
            current: 'hsl(0, 70%, 55%)',
            currentGlow: 'hsla(0, 70%, 55%, 0.35)',
            unlocked: 'hsl(270, 35%, 45%)',
            explored: 'hsl(270, 40%, 55%)',
            locked: 'hsl(270, 15%, 18%)',
            lockedText: 'hsl(270, 15%, 42%)',
            path: 'hsl(270, 20%, 25%)',
            pathLocked: 'hsl(270, 15%, 15%)',
        }
    };
    ...
    // ═══════════════════════════════════════════════════════════
    // 6. 样式注入（完整保留，只改了一处：.page-container 加 min-height 兜底）
    // ═══════════════════════════════════════════════════════════
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            .map-system-panel .map-header .title {
                font-size: 0.9rem; font-weight: 700; color: var(--map-text-primary); letter-spacing: 3px;
            }
            .map-system-panel .map-header .title span {
                font-weight: 400; color: var(--map-text-secondary); font-size: 0.65rem; letter-spacing: 1px;
            }
            ...
            .map-system-panel .page-container {
                position: relative; overflow: hidden;
                flex: 1 1 0;
                min-height: 55vh;  /* ← 关键修复：兜底最小高度，防止 flex 计算失败时为 0 */
                height: auto;
            }
            .map-system-panel .pages-wrapper {
                display: flex; height: 100%;
                transition: transform 0.5s cubic-bezier(0.34,1.0,0.64,1);
            }
            .map-system-panel .map-page {
                flex: 0 0 100%;
                overflow-y: auto;
                padding: 4px 2px 8px;
                display: flex;
            ...
            .map-system-panel .location-node .node-ring {
            ...
            .map-system-panel .location-node .node-label {
                font-size: 0.5rem; text-align: center; letter-spacing: 0.5px; line-height: 1.2;
                transition: all 0.3s ease; max-width: 50px; word-break: break-all; color: var(--map-text-secondary);
            }
            .map-system-panel .location-node .sub-badge {
                font-size: 0.3rem; color: var(--map-text-muted); background: rgba(200,170,190,0.04);
                padding: 0 5px; border-radius: 6px; border: 1px solid var(--map-panel-border);
            }

            .map-system-panel .map-actions {
                display: flex; justify-content: center; gap: 12px; padding: 8px 0 2px;
                margin-top: 6px; border-top: 1px solid var(--map-panel-border); flex-wrap: wrap; align-items: center; flex-shrink: 0;
            }
            .map-system-panel .map-actions .action-btn {
                padding: 5px 18px; border: 1px solid var(--map-panel-border); border-radius: 18px;
                background: rgba(200,170,190,0.02); color: var(--map-text-secondary);
                font-size: 0.6rem; font-family: inherit; letter-spacing: 1px; cursor: pointer; transition: all 0.3s ease;
            }
            ...
            .map-system-panel .map-legend {
                display: flex; justify-content: center; gap: 10px; padding: 6px 0 2px;
                border-top: 1px solid var(--map-panel-border); margin-top: 4px; flex-wrap: wrap; flex-shrink: 0;
            }
            ...
            .map-system-panel .map-footer-hint {
                display: flex; justify-content: center; padding-top: 4px; border-top: 1px solid var(--map-panel-border);
                margin-top: 2px; flex-shrink: 0;
            }
            .map-system-panel .map-footer-hint span { font-size: 0.38rem; color: var(--map-text-muted); letter-spacing: 1px; }

            .map-system-panel .detail-layout {
                display: flex; flex-direction: row; gap: 16px; align-items: flex-start; padding: 4px 0;
            }
            .map-system-panel .detail-layout .detail-image {
                flex: 0 0 40%; max-width: 200px; border-radius: 14px; overflow: hidden;
                box-shadow: 0 4px 24px rgba(0,0,0,0.4); border: 1px solid var(--map-panel-border);
                background: rgba(200,170,190,0.03); aspect-ratio: 3/4;
                display: flex; align-items: center; justify-content: center; color: var(--map-text-muted); font-size: 2.5rem;
            }
            ...
            .map-system-panel .detail-layout .detail-info .detail-desc,
            #mapSubDesc {
                font-size: 0.7rem;
            ...
            .map-system-panel .clickable-tag {
                display: inline-block; padding: 3px 12px; border: 1px solid var(--map-panel-border); border-radius: 14px;
                font-size: 0.5rem; color: var(--map-text-secondary); background: rgba(200,170,190,0.02);
                cursor: pointer; transition: all 0.3s ease; font-family: inherit; margin: 2px 4px 2px 0;
            }
            ...
            .map-system-panel .typing-cursor {
                display: inline-block; width: 2px; height: 0.8em; background: var(--map-node-current);
                margin-left: 2px; vertical-align: text-bottom; animation: mapBlink 0.8s step-end infinite;
            }
            ...
            .map-system-panel .detail-actions .act-btn {
                padding: 4px 14px; border: 1px solid var(--map-panel-border); border-radius: 16px;
                background: rgba(200,170,190,0.02); color: var(--map-text-secondary);
                font-size: 0.55rem; font-family: inherit; cursor: pointer; transition: all 0.3s ease; letter-spacing: 1px;
            }
            ...
            .map-system-panel .detail-sub-label {
                font-size: 0.5rem; color: var(--map-text-muted); letter-spacing: 1px;
                padding: 2px 8px; border-radius: 10px; background: rgba(200,170,190,0.04); border: 1px solid var(--map-panel-border);
            }

            .map-system-toast {
                position: fixed; bottom: 30px; left: 50%; transform: translateX(-50%);
                background: rgba(20,16,20,0.85); backdrop-filter: blur(16px); padding: 8px 20px; border-radius: 20px;
                border: 1px solid var(--map-panel-border); color: var(--map-text-secondary);
                font-size: 0.7rem; letter-spacing: 1px; z-index: 999999;
                font-family: 'Georgia','Times New Roman',serif; transition: all 0.3s ease; opacity: 0;
                animation: mapToastIn 0.4s cubic-bezier(0.34,1.0,0.64,1) forwards;
            }
            ...
            @media (max-width: 640px) {
                .map-system-panel .map-card {
                    width: 96vw; padding: 10px 10px 8px; border-radius: 20px; max-height: 96vh;
                }
    ...
    function getStatData() {
        try { const allVars = getAllVariables(); return _.get(allVars, 'stat_data', {}); } catch (e) { return {}; }
    }
        ...
        for (let r = 0; r < 5; r++) {
            for (let c = 0; c < 5; c++) {
                ...
                if (node) {
                    ...
                    html += `
                        <div class="location-node ${statusClass} ${isSelected ? 'selected' : ''} ${isLit ? 'lit' : ''}"
                             data-id="${node.id}">
                            <div class="node-ring"><span class="inner"></span></div>
                            <span class="node-label">${node.id}</span>
                            ${hasSubs ? `<span class="sub-badge">${subCount} 处</span>` : ''}
                        </div>`;
                } else {
    ...
    function renderPaths() {
        ...
        neighbors.forEach(nId => {
            ...
            svgContent += `
                <line x1="${cp.x}" y1="${cp.y}" x2="${np.x}" y2="${np.y}"
                      stroke="${strokeColor}" stroke-width="${isLocked ? 1.5 : 2.5}"
                      opacity="${opacity}" stroke-dasharray="${dash}" />`;
        });
        const extraPairs = [
            ['公园', '大学校园'], ['大学校园', '苏小薇家'], ['河边散步道', '咖啡厅'],
            ['赵雅兰·赵梦琪家', '咖啡厅'], ['公司', '超市'], ['超市', '电影院'],
            ['游乐场', '海边']
        ];
        extraPairs.forEach(pair => {
            ...
            svgContent += `
                <line x1="${pa.x}" y1="${pa.y}" x2="${pb.x}" y2="${pb.y}"
                      stroke="var(--map-path-locked)" stroke-width="1" opacity="0.06" stroke-dasharray="2 4" />`;
        });
        svgContent += `
            <circle cx="${cp.x}" cy="${cp.y}" r="16" fill="none"
                    stroke="var(--map-node-current)" stroke-width="1.5" opacity="0.25">
            ...
            <line x1="${cp.x - 7}" y1="${cp.y}" x2="${cp.x + 7}" y2="${cp.y}"
                  stroke="var(--map-node-current)" stroke-width="1.5" opacity="0.5"/>
            <line x1="${cp.x}" y1="${cp.y - 7}" x2="${cp.x}" y2="${cp.y + 7}"
                  stroke="var(--map-node-current)" stroke-width="1.5" opacity="0.5"/>`;
    ...
    function renderL1() {
        ...
        setTimeout(() => {
            ...
            if (selectedNodeId) {
                const selEl = gridElement?.querySelector(`.location-node[data-id="${selectedNodeId}"]`);
                if (selEl && isNodeInteractable(selectedNodeId)) selEl.classList.add('selected');
...
// ============================================================
// 第二部分：L2/L3详情、构建UI（含手机自适应）、API、按钮注册
// ============================================================
        ...
        if (detailGoBtn) {
            ...
            detailGoBtn.onclick = () => {
                if (status !== 'locked' && !isCurrent) {
                    ...
                    gridElement?.querySelectorAll('.location-node.selected').forEach(n => n.classList.remove('selected'));
                    updateMVULocation(id); goToPage(0); renderL1(); showToast('📍 已前往 "' + id + '"');
                }
            };
        }
        ...
        if (subGoBtn) {
            ...
            subGoBtn.onclick = () => {
                if (parentStatus !== 'locked' && !isCurrentParent && parentId) {
                    ...
                    gridElement?.querySelectorAll('.location-node.selected').forEach(n => n.classList.remove('selected'));
                    updateMVULocation(parentId); goToPage(0); renderL1(); showToast('📍 已前往 "' + parentId + '"');
                }
            };
        }
    ...
    // ═══════════════════════════════════════════════════════════
    // 15. 构建 UI（已添加手机端高度自适应 + min-height 兜底）
    // ═══════════════════════════════════════════════════════════
    ...
    function buildUI() {
        ...
        panel.innerHTML = `
            <div class="map-card">
                ...
                <div class="location-bar">
                    ...
                    <span class="name" id="mapLocationName">公寓</span>
                </div>
                <div class="compass">
                    <span class="dir active">北</span><span class="line"></span>
                    <span class="dir">东北</span><span class="line"></span>
                    <span class="dir">东</span><span class="line"></span>
                    <span class="dir">东南</span><span class="line"></span>
                    <span class="dir">南</span><span class="line"></span>
                    <span class="dir">西南</span><span class="line"></span>
                    <span class="dir">西</span><span class="line"></span>
                    <span class="dir">西北</span>
                </div>
                <div class="page-container">
                    <div class="pages-wrapper" id="mapPagesWrapper">
                        <div class="map-page" id="mapPageL1">
                            <div class="map-canvas fade-in" id="mapCanvas">
                                <div class="path-overlay" id="mapPathOverlay">
                                    <svg viewBox="0 0 580 460" preserveAspectRatio="xMidYMid meet" id="mapPathSvg"></svg>
                                </div>
                            ...
                            <div class="map-footer-hint">
                                <span>单击选中 · 双击进入详情 · 红色菱形为当前位置 · 周围节点已点亮</span>
                                <span>作者 · 三年的水</span>
                            </div>
                        </div>
                        <div class="map-page" id="mapPageDetail">
                            <div class="page-title">
                                <button class="back-btn" id="mapBackBtn">‹</button>
                                <span class="page-label">地点 ·</span>
                                <span class="page-name" id="mapDetailName">公寓</span>
                            </div>
                        ...
                        <div class="map-page" id="mapPageSubDetail">
                            <div class="page-title">
                                <button class="back-btn" id="mapSubBackBtn">‹</button>
                                <span class="page-label" id="mapSubLabel">子场景 ·</span>
                                <span class="page-name" id="mapSubName">客厅</span>
                            </div>
        ...
        function adjustPanelHeight() {
            ...
            if (h > 0) {
                ...
                if (card) {
                    card.style.maxHeight = Math.round(h * 0.9) + 'px';
                }
            }
        }
        adjustPanelHeight();
        ...
        const topWin = getTopWindow();
        const resizeHandler = function() { adjustPanelHeight(); };
        topWin.addEventListener('resize', resizeHandler);
        topWin.addEventListener('orientationchange', function() { setTimeout(adjustPanelHeight, 300); });
        ...
        try {
            if (typeof eventOn !== 'undefined' && typeof Mvu !== 'undefined') {
                eventOn(Mvu.events.VARIABLE_UPDATE_ENDED, (newVars) => {
                    if (!isMounted) return;
                    const newScene = _.get(newVars, 'stat_data.世界.当前场景');
                    if (newScene && newScene !== currentLocationId && NODES.some(n => n.id === newScene)) {
    ...
    topWin.__mapSystem = {
        ...
        refresh: () => { if (isMounted) { refreshNodeCache(); renderL1(); } },
        moveTo: (id) => {
            ...
            if (node && isNodeInteractable(id)) {
                ...
                currentLocationId = id; selectedNodeId = null;
                gridElement?.querySelectorAll('.location-node.selected').forEach(n => n.classList.remove('selected'));
                updateMVULocation(id); renderL1(); showToast('📍 已前往 "' + id + '"');
    ...
    try {
        ...
        topWin.__mapSystemOpen = openMap;
    }

    $(window).on('pagehide', () => { if (isMounted) closeMap(); });
    console.log('[地图] 系统已初始化，点击 "🗺️ 都市地图" 按钮打开');
